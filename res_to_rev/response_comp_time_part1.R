##########################
# modified from gen_sim_data_0811.R in real_data_sim
# it generate a dataset with larger sample size for assessing the computational efficiency
##########################
library(tidyverse)
library(MASS)
library(doParallel)
...
# parameters <- "VNCELL=FALSE;DIFF=1.1;NCELL=1000;NPG=50;BOOT=1"
parameters <- "VNCELL=FALSE;DIFF=1.1;NCELL=1000;NPG=200;BOOT=1"
...
tryCatch({
  for(paras in parameters){
    ...
    for(b in 1:boot){ # for FDP calculation, no set.seed inside this loop
      ...
      save(dat_list, file = 
             str_glue("/home/guanwh/zhan7474/mayo/res_to_rev/n{n_cas}_{n_ctr}_d{diff}_c{n_cell}_v{neq_n_cell}_{dddd}_b{b}.RData"))
