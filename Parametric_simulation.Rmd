...
read_file("source_code/exam_sim_data.sh") %>% 
  ...
  write_file(file = str_glue("para_data_sim/exam_sim_data_{CAT}_{DATE}.sh"), 
             append = FALSE)
# WORKINGDIRECTORY, NCORE, NLIM_1, NLIM_2, 
# PARA_OUT, SIMU_D, CAT
read_file("source_code/exam_sim_data.R") %>% # no DATE but SIMU_D!!!
  ...
  str_replace_all(pattern = "PARA_OUT", PARA_OUT) %>%
  str_replace_all(pattern = "CAT", CAT) %>%
  write_file(file = str_glue("para_data_sim/exam_sim_data_{CAT}_{DATE}.R"), 
             append = FALSE)
...
-  Example shell script (`r str_glue("para_data_sim/exam_sim_data_{CAT}_{DATE}.sh")`)

```{bash, file = str_glue("para_data_sim/exam_sim_data_{CAT}_{DATE}.sh"), eval = FALSE}
```

-  Example R script (`r str_glue("para_data_sim/exam_sim_data_{CAT}_{DATE}.R")`)

```{r, file = str_glue("para_data_sim/exam_sim_data_{CAT}_{DATE}.R"), eval = FALSE}
```

## Examining the Association between Treatment and Transcriptomic profiles using Different Methods

Run the scripts above on a supercomputer to test the association between phenotype and single-cell RNA sequencing data. The results will be saved as 'results_n{n_cas}_{n_ctr}_d{diff}_c{n_cell}v{neq_n_cell}{DATE}.RData.'"
...
for(paras in parameters){
  ...
  if(!file.exists(str_glue("para_data_sim/results_{conf}.RData")))
    next
  load(str_glue("para_data_sim/results_{conf}.RData")) # "disc_pval", "ideas_pval", "deseq_pval"
  cat(str_glue("loaded {conf}\n"))
  
  pval_list <- list(disc_pval, ideas_pval, deseq_pval)
