##########################
#This file is to restart the previously stopped job
#So the script need to be re-written according to the specific error message
##########################

load(str_glue("unfinished_{conf}.RData"))
# stopped at
#disc_pval <- parSapply(cl, 1:boot, simplify = FALSE, FUN = function(bbbb){
# load(str_glue("{conf}_b{bbbb}.RData"))
# it has previously-saved random seed, "DiSC", "paras" etc.
