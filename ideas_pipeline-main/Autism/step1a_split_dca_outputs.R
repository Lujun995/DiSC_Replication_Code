# modified from https://github.com/Sun-lab/ideas_pipeline/blob/main/Autism/step1a_split_dca_outputs.R
setwd("/home/guanwh/zhan7474/mayo/ideas_pipeline-main/Autism/")
lab = "PFC_all"
data.dir = "./data"
...
split_cluster <- function(dat, cell_info, statistics){
  ...
  for(c1 in uClusters){
    ...
    c1 = gsub("\\/", "_", c1)
    fnm = file.path(sprintf("dca_%s/%s_%s.tsv", lab, c1, statistics))
...
dca_mean  = fread(file.path(sprintf("dca_%s/mean_norm.tsv", lab)))
gc()
dim(dca_mean)
dca_mean[1:2,1:5]

split_cluster(dca_mean, cell_info, "mean_norm")

gc()

# second is the pi parameter
dca_pi = fread(file.path(sprintf("dca_%s/dropout.tsv", lab)))
if(sum(dim(dca_mean) != dim(dca_pi)) == 0) names(dca_pi) <- names(dca_mean)
gc()
dim(dca_pi)
dca_pi[1:2,1:5]
...
dca_theta = fread(file.path(sprintf("dca_%s/dispersion.tsv", lab)))
if(sum(dim(dca_mean) != dim(dca_theta)) == 0) names(dca_theta) <- names(dca_mean)
gc()
dim(dca_theta)
dca_theta[1:2,1:5]
